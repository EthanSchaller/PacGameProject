...
public class Ghosts extends Sprite implements Runnable{
	final static int SOCKET_PORT = 6000;
	...
	public int[] shdwMv(int[] GXY, int[] PXY) throws UnknownHostException, IOException {
		Socket s = null;
		...
		if(GXY[1] > PXY[1]) {
			...
			String command = "shdwL";
			out.println(command);
			out.flush();
			int commandI = this.getX();
			out.println(commandI);
			out.flush();
			commandI = this.getY();
			out.println(commandI);
			out.flush();
	...
	public int[] prmMv(int[] GXY, String mv) throws UnknownHostException, IOException {
		Socket s = null;
	...
	public int[] nrmMv(int[] GXY, String mv, int timer) throws IOException {
		Socket s = null;
	...
	public int[] eatMv(int[] GXY) throws IOException {
		Socket s = null;
		s = new Socket("localhost", SOCKET_PORT);
	...
	public void run() {
		...
		while(Move) {
			...
			if(!Eaten) {
				switch(GhNum) {
					case 1: try {
						GhostXY = nrmMv(GhostXY, mvDir[GhNum], intrnlTimer);
					...
					case 3: try {
						GhostXY = prmMv(GhostXY, mvDir[GhNum]);
	...
	public boolean plyrCollision() {
		...
		if(this.rect.intersects(Pac.getRect())) {
			...
			if(Vul == false && Eaten == false) {
				...
				//pacman is set to its death animation and set to the center of the screen
