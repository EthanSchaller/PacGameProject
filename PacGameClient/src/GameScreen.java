...
public class GameScreen extends JFrame  implements ActionListener, KeyListener {
	...
	final static int SOCKET_PORT = 5001;
	...
	public void keyPressed(KeyEvent e) {
		...
		if(Hit == false && !ExitBttn.isVisible() && PacLbl.isVisible()) {
			...
			//TODO send location in each movement after the direction commands
			...
			if(e.getKeyCode() == KeyEvent.VK_W || e.getKeyCode() == KeyEvent.VK_UP) {
				...
				out.println(command);
				out.flush();
				int commandI = PacLbl.getX();
				out.println(commandI);
				out.flush();
				commandI = PacLbl.getY();
				out.println(commandI);
				out.flush();
...
//				lbCreate();
	...
	public void  loseScreen() throws SQLException, IOException {
		Socket s = null;
		...
		for(int i = 1; i <= 5; i++) {
			Disp += "           " + in.next() + " "  + in.next() + " " + in.next() + " " + in.nextInt() + "\r\n";
		}
	...
	public void  winScreen() throws SQLException, IOException {
		Socket s = null;
		
		StartBttn.setVisible(true);
		StartBttn.setText("LBdisp");
		startScreen();
